...
export default function Footer() {
  ...
  return (
    <footer className="bg-[#050505] text-[#F5F5F5] pt-12 sm:pt-24 pb-20 sm:pb-12 border-t border-white/10 relative overflow-hidden font-sans">
      {/* Ambient Yellow Background Glow Accent */}
      ...
      <div className="container mx-auto px-5 sm:px-6 relative z-10">
        {/* Editorial Headline */}
        <div className="mb-10 sm:mb-16 pb-8 border-b border-white/10 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <span className="text-[10px] sm:text-xs font-mono tracking-[0.3em] text-[#FFD400] uppercase font-bold">
              SON SÖZ & DAVET
            </span>
            <h2 className="font-display font-extrabold text-2xl sm:text-5xl text-white mt-1 leading-tight tracking-tight">
              Aracınız için <br className="hidden sm:inline" />
          ...
          <div>
            <Link
              href="/randevu"
              className="w-full sm:w-auto px-6 sm:px-8 py-3.5 bg-[#FFD400] text-[#050505] font-extrabold text-xs uppercase tracking-widest hover:bg-white transition-colors flex items-center justify-center gap-2 text-center"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 sm:gap-12 mb-10 sm:mb-16">
              ...
              {socialLinks.length > 0 ? (
                socialLinks.map((link) => (
                  <a
                    ...
                    className="w-9 h-9 rounded-full border border-white/10 glass flex items-center justify-center hover:border-[#FFD400] transition-all"
                  >
                    <SocialBrandIcon platform={link.platform} size={16} />
                  </a>
                ))
              ) : (
                <>
                  ...
                  <a
                    ...
                    className="w-9 h-9 rounded-full border border-white/10 glass flex items-center justify-center hover:border-[#FFD400] transition-all"
                  >
                    <SocialBrandIcon platform="Telefon" size={16} />
                  </a>
                  <a
                    ...
                    className="w-9 h-9 rounded-full border border-white/10 glass flex items-center justify-center hover:border-[#FFD400] transition-all"
                  >
                    <SocialBrandIcon platform="E-Posta" size={16} />
                  </a>
                </>
              )}
            </div>
          ...
          <div className="flex flex-col gap-3">
            <h3 className="font-mono text-xs font-bold text-[#FFD400] uppercase tracking-widest pb-1 border-b border-white/5 sm:border-0">
              HİZMETLER
            </h3>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex items-start gap-2 text-xs text-gray-300 font-light leading-relaxed">
              <MapPin size={14} className="text-[#FFD400] shrink-0 mt-0.5" />
              <span>{settings.fullAddress || "Limonluk Mahallesi, 18. Cadde, No: 76/A, 33011 Yenişehir / Mersin"}</span>
            </div>
            ...
            <div className="flex items-center gap-2 text-xs text-gray-300 font-light">
              <Mail size={14} className="text-[#FFD400] shrink-0" />
              <a href={`mailto:${settings.email}`} className="hover:text-[#FFD400] font-mono transition-colors">
                {settings.email}
              </a>
            </div>
            <div className="pt-2 text-[11px] font-mono text-gray-400 border-t border-white/10">
              {settings.workingHours}
            </div>
          </div>
        </div>

        {/* Bottom Boxed Section with 2-by-2 Grid Legal Links & HWK Monogram Logo */}
        <div className="p-5 sm:p-6 bg-white/[0.02] border border-white/10 rounded-2xl flex flex-col lg:flex-row items-center justify-between gap-5 text-center lg:text-left font-mono text-xs">
          ...
          {/* Legal Links in 2-by-2 Grid */}
          <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-center text-[10px] sm:text-[11px] text-gray-400 max-w-sm">
          ...
          {/* Corporate HWK Geometric Monogram Logo */}
          <HWKDigitalLogo />
        </div>
      </div>
    </footer>
